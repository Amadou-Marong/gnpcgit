<div class="bg-white p-3">
    <h4>Exam Name: {{$exam->exam_name}}</h4>
    <p>Per question mark: {{$exam->per_question_mark}}</p>
    <p>Total marks: {{$exam->per_question_mark * $exam->questions->count()}}</p>
    @if ($answers->count() > 0)
    ...
    <form method="POST" action="{{ asset('/course/'.$exam->id.'/exam') }}">
        {{ csrf_field() }}
        ...
        @foreach ($exam->questions->where('long_written',0)->where('missing_word',0) as $questions)
        ...
        @endforeach
        @foreach ($exam->questions->where('missing_word',1) as $questions)
        ...
        @endforeach
        @foreach ($exam->questions->where('long_written',1) as $questions)
    ...
    @else
    <p class="text-danger text-center mt-5">No questions in this Exam</p>
    @endif
